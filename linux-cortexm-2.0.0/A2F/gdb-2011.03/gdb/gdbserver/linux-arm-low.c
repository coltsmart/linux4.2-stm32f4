...
#if defined (__LINUX_CORTEX_M)
void init_registers_arm_linux_cortex_m (void);
#endif
...
arm_breakpoint_at (CORE_ADDR where)
{
  ...
  collect_register_by_name (regcache, "xpsr", &xpsr);
...
arm_arch_setup (void)
{
  ...
  if (arm_get_hwcap (&arm_hwcap) == 0)
    ...
    init_registers_arm ();
#else
    init_registers_arm_linux_cortex_m ();
#endif
  else if (arm_hwcap & HWCAP_IWMMXT)
    init_registers_arm_with_iwmmxt ();
  else if (arm_hwcap & HWCAP_VFP)
    {
      int pid;
      char *buf;
...
#ifndef __LINUX_CORTEX_M
#ifdef HAS_NOMMU
  /* Modify the XML file to report the uClinux OSABI.  */
  {
    static const char osabi[] = "\n  <osabi>uClinux</osabi>";
    static char *new_xml;
    char *current_xml = strdup (get_features_xml ("target.xml"));
    char *p = strstr (current_xml, "<target>");
    ...
    p = p + strlen ("<target");
    *p++ = '\0';
    new_xml = malloc (strlen (current_xml) + strlen (osabi)
		      + strlen (p) + 3);
    sprintf (new_xml, "@%s>%s%s", current_xml, osabi, p);
    gdbserver_xmltarget = new_xml;
  }
#endif
#endif /* __LINUX_CORTEX_M */
}
